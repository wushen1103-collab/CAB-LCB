...
def main() -> None:
    ...
    for exp_dir in iter_experiment_dirs(runs_dir):
        ...
        base_info: Dict[str, Any] = {
            ...
            "exp_name": exp_name,
            "exp_dir": str(exp_dir),
            "dataset": exp_metrics.get("dataset"),
            "split": exp_metrics.get("split"),
            "seed": exp_metrics.get("seed"),
    ...
    # Selection on eval rows (cal_cp protocol)
    ...
    if df_eval.empty:
        ...
        def sort_key(frame: pd.DataFrame) -> pd.DataFrame:
            ...
            frame["gamma_val"] = frame["gamma_val"].apply(lambda x: float(x) if x is not None else 0.0)
            frame["k_val"] = frame.get("k_neighbors")
            frame["k_val"] = frame["k_val"].apply(lambda x: int(x) if x is not None else 10**9)
            ...
            # Prefer: cov_ok=1, then smallest width, then smallest gamma, then smaller k/K
        ...
        # Attach matching test metrics if present
